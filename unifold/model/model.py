...
from unifold.common import confidence
from unifold.model import features
from unifold.model import modules
from unifold.train.data_system import cast_to_precision
from unifold.train.mixed_precision import normalize_precision, cast_floating_to
import haiku as hk
import jax
...
class RunModel:
    ...
    def __init__(self,
        ...
        def _forward_fn(batch):
            ...
            return cast_floating_to(ret, jnp.float32)
    ...
    def cast_params(self):
        ...
        if self.precision == 'fp16':
            self.params = cast_floating_to(self.params, jnp.float16)
        elif self.precision == 'bf16':
            self.params = cast_floating_to(self.params, jnp.bfloat16)
    ...
    def process_features(
        ...
        if isinstance(raw_features, dict):
            batch = features.np_example_to_features(
                np_example=raw_features,
                config=self.config,
                random_seed=random_seed)
        else:
            batch = features.tf_example_to_features(
                tf_example=raw_features,
                config=self.config,
                random_seed=random_seed)
        return cast_to_precision(batch, self.precision)
