...
from unifold.model import common_modules
from unifold.model import folding
from unifold.model import layer_stack
from unifold.model import layer_norm
from unifold.model import lddt
from unifold.model import mapping
...
def masked_by_dtype(dtype, x):
    # dtype is known at compile time, so use if here is okay
...
def softmax_cross_entropy(logits, labels):
    ...
    loss = -jnp.sum(labels * jax.nn.log_softmax(logits.astype(jnp.float32)), axis=-1)
    ...
    def __call__(self,
        ...
        if ensemble_representations:
            def body(x):
                ...
                return new_representations, i + 1
            ...
            if hk.running_init():
                ...
                # altered by weijie for training purpose.
                # representations, _ = hk.scan(body, representations, jnp.arange(1, num_ensemble))
                # original code:
                representations, _ = hk.while_loop(
                    lambda x: x[1] < num_ensemble,
                    body,
                    (representations, 1))
...
#
# class ProbAttention(hk.Module):
#     """Prob sparse attention"""
#
#     def __init__(self, config, global_config, output_dim, factor=5, name="prob_attention"):
#         super().__init__(name=name)
#
#         self.config = config
#         self.global_config = global_config
...
#         _, _, s, _ = q.shape
#
#         k_expand = jnp.broadcast_to(jnp.expand_dims(k, -3), (b, h, s, l, e))
#         indx_q_seq = tf.random.uniform((S,), maxval=L, dtype=tf.int32)
#         indx_k_seq = tf.random.uniform((sample_k,), maxval=L, dtype=tf.int32)
#
#         K_sample = jnp.gather(k_expand, jnp.range(S), axis=2)  # B, H, S, L, E
#
#         K_sample = tf.gather(K_sample, indx_q_seq, axis=2)  # B, H, S, L, E
#         K_sample = tf.gather(K_sample, indx_k_seq, axis=3)  # B, H, S, u, E
#
#         # (B, H, S, 1, E) * (B, H, S, u, E)
#         Q_K_sample = tf.squeeze(tf.matmul(jnp.expand_dims(Q, -2), tf.einsum("...ij->...ji", K_sample)))
#         # B, H, S, u
#         # find the Top_k query with sparisty measurement
#         M = tf.math.reduce_max(Q_K_sample, axis=-1) - tf.raw_ops.Div(x=tf.reduce_sum(Q_K_sample, axis=-1), y=L)
#         # B, H, S
#
#         M_top = tf.math.top_k(M, n_top, sorted=False)[1]  # B, H, U
...
#
#         idx = tf.stack(values=[batch_indexes, head_indexes, M_top], axis=-1)  # B, H, U, 3
#
#         # use the reduced Q to calculate Q_K
#         Q_reduce = tf.gather_nd(Q, idx)  # B, H, U, D
#
...
class GlobalAttention(hk.Module):
    ...
    def __call__(self, q_data, m_data, q_mask, bias):
        ...
        if q_data.dtype == jnp.float16:
            q_avg = utils.mask_mean(q_mask, q_data.astype(jnp.float32), axis=1, eps=1e-5).astype(q_data.dtype)
        else:
...
class MSARowAttentionWithPairBias(hk.Module):
    ...
    def __call__(self,
        ...
        bias = masked_by_dtype(msa_act.dtype, (msa_mask - 1.))[:, None, None, :]
        assert len(bias.shape) == 4

        msa_act = layer_norm.LayerNorm(
            axis=[-1], create_scale=True, create_offset=True, name='query_norm')(
            msa_act)

        pair_act = layer_norm.LayerNorm(
            axis=[-1],
            create_scale=True,
            create_offset=True,
            name='feat_2d_norm')(
...
class TriangleAttention(hk.Module):
    ...
    def __call__(self, pair_act, pair_mask, is_training=False):
        ...
        bias = masked_by_dtype(pair_act.dtype, (pair_mask - 1.))[:, None, None, :]
        assert len(bias.shape) == 4
...
class TriangleMultiplication(hk.Module):
    ...
    def __call__(self, act, mask, is_training=True):
        ...
        act = layer_norm.LayerNorm(axis=[-1], create_scale=True, create_offset=True,
...
class OuterProductMean(hk.Module):
    ...
    def __call__(self, act, mask, is_training=True):
        ...
        mask = mask[..., None]
        act = layer_norm.LayerNorm([-1], True, True, name='layer_norm_input')(act)
...
class EmbeddingsAndEvoformer(hk.Module):
    ...
    def __call__(self, batch, is_training, safe_key=None):
        ...
        if c.recycle_pos and 'prev_pos' in batch:
            ...
            dgram = dgram_from_positions(prev_pseudo_beta, **self.config.prev_pos).astype(pair_activations.dtype)
        ...
        if c.recycle_features:
            if 'prev_msa_first_row' in batch:
                prev_msa_first_row = layer_norm.LayerNorm([-1],
                                                          True,
                                                          True,
            ...
            if 'prev_pair' in batch:
                prev_pair = batch['prev_pair'].astype(pair_activations.dtype)
                pair_activations += layer_norm.LayerNorm([-1],
                                                         True,
                                                         True,
                                                         name='prev_pair_norm')(
                    prev_pair)
        ...
        if c.max_relative_feature:
            ...
            rel_pos = jax.nn.one_hot(
                ...
                2 * c.max_relative_feature + 1).astype(pair_activations.dtype)
            pair_activations += common_modules.Linear(
        ...
        extra_msa_feat = create_extra_msa_feature(batch).astype(msa_activations.dtype)
        ...
        if c.template.enabled and c.template.embed_torsion_angles:
            ...
            template_features = jnp.concatenate([
                aatype_one_hot,
                jnp.reshape(
                    ret['torsion_angles_sin_cos'].astype(msa_activations.dtype), [num_templ, num_res, 14]),
                jnp.reshape(
                    ret['alt_torsion_angles_sin_cos'].astype(msa_activations.dtype), [num_templ, num_res, 14]),
                ret['torsion_angles_mask'].astype(msa_activations.dtype)], axis=-1)
...
class SingleTemplateEmbedding(hk.Module):
    ...
    def __call__(self, query_embedding, batch, mask_2d, is_training):
        ...
        template_mask = batch['template_pseudo_beta_mask'].astype(dtype)
        ...
        act = layer_norm.LayerNorm([-1], True, True, name='output_layer_norm')(act)
        return act
...
class TemplateEmbedding(hk.Module):
    ...
    def __call__(self, query_embedding, template_batch, mask_2d, is_training):
        ...
        bias = masked_by_dtype(dtype, template_mask[None, None, None, :] - 1.)
