...
"""Feature pre-processing input pipeline for Uni-Fold."""

from unifold.model.tf import data_transforms
from unifold.model.tf import shape_placeholders
...
def process_tensors_from_config(tensors, data_config):
    ...
    def wrap_ensemble_fn(data, i):
        ...
        # ziyao: i is a tf.Tensor.
        d = data.copy()
        fns = ensembled_map_fns(data_config)
        fn = compose(fns)
        # ziyao: add an index for the ensemble branch.
        d['ensemble_index'] = i
        return fn(d)
    ...
    if data_config.common.resample_msa_in_recycling:
        ...
        # ziyao: the total num of ensemble branches: N = Ne * (Nc + 1)
    ...
    if isinstance(num_ensemble, tf.Tensor) or num_ensemble > 1:
        ...
        # ziyao: establish all ensemble branches once and for all.
        tensors = tf.map_fn(
            lambda x: wrap_ensemble_fn(tensors, x),  # ziyao: x the index = 0, 1, ..., N-1
            tf.range(num_ensemble),  # ziyao: the indices xs.
...
def compose(x, fs):
    # ziyao: forms a pipeline by sequentially applying a list of functions on x.
    #        as it is decorated by curry1, this func shall be used as compose(fns)(x).
    for f in fs:
        x = f(x)
    return x
